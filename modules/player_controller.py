...
from modules.config import max_speed, gravity, jump_power, game_stats, SCREEN_HEIGHT, SCREEN_WIDTH, level_paths, player_size
...
class PlayerController:
    ...
    def animate(self:object, keys:list, delta_time:float, current_level:int):
        ...
        if self.has_hit_wall_midair:
            ...
            if not bounce_channel.get_busy() and self.has_collided_this_frame:
                bounce_channel.play(sfx["bounce"])
    ...
    def move(self:object, delta_time:float, keys:list, current_level:int, level:list, levels:list):
        ...
        if self.touched_floor and not any(keys):
            self.speed_x *= 0.5
        if self.speed_x > max_speed and self.touched_floor:
            self.speed_x = max_speed
        elif self.speed_x < max_speed * -1 and self.touched_floor:
            self.speed_x = max_speed * -1
        ...
        if not self.touched_floor:
            if self.speed_y <= 0:
                self.speed_y += gravity * delta_time
            else: 
                self.speed_y += gravity * 2 * delta_time
        ...
        self.check_collision_with_platforms(current_level, level)
        ...
        if (keys[pygame.K_UP] or keys[pygame.K_w] or keys[pygame.K_SPACE]) and self.time_charged <= 2:
            if self.touched_floor:
                ...
                if self.jump_charge < jump_power:
                    self.jump_charge = jump_power
        ...
        self.animate(keys, delta_time, current_level)
    ...
    def check_borders(self:object, current_level:int, level:list, levels:list):
        ...
        if self.rect.right > SCREEN_WIDTH: # RIGHT
            ...
            if self.touched_floor:
                ...
                sfx["bounce"].play()
                return -0.5, level, current_level
        ...
        if self.rect.centery > SCREEN_HEIGHT: # BOTTOM
            current_level -= 1
            if not current_level < 0:
                ...
                current_level += 1
                self.reset_position((SCREEN_WIDTH / 2) - (player_size / 2), SCREEN_HEIGHT - 400)
                
        if self.rect.centery < 0: # TOP
            current_level += 1
            if not current_level >= len(level_paths):
                self.reset_position(self.rect.x, SCREEN_HEIGHT - self.rect.size[0])
                level = levels[current_level]
            else:
                self.speed_y = 1
                self.rect.centery = 1
                ...
                if self.collisions[0]:
                    ...
                    if not self.has_collided_prev_frame: # landing detection, sfx, orientation
                        ...
                        if self.speed_x > 0:
                            ...
                            self.direction = "left"
                        if (self.rect.y - 20 > self.jumped_from_y and (self.times_bounced_midair >= 2 or self.head_bounce)) or self.jumped_from_level > current_level:   # pravila za fall
                            self.fell = True
                            sfx["fall"].play()
                            game_stats["falls"] += 1
                            game_stats["fall_distance"] += abs(self.jumped_from_y - self.rect.y)
                        else:
                            ...
                            if self.has_received_input:
                                ...
                                if self.jumped_from_y - self.rect.y > 5:
                                    game_stats["distance_climbed"] += self.jumped_from_y - self.rect.y + 1
                                
                                elif current_level > self.jumped_from_level: #ce skocimo v nov level
                                    game_stats["distance_climbed"] += self.jumped_from_y + (1000 - self.rect.y) + 1
                                
                                elif self.jumped_from_y - self.rect.y < -5:
                                    game_stats["distance_descended"] += abs(self.jumped_from_y - self.rect.y) - 1

                                game_stats["wall_bounces"] += self.times_bounced_midair
                                if current_level + 1 > game_stats["best_screen"]:
                                    game_stats["best_screen"] = current_level + 1
                        self.speed_x = 0
